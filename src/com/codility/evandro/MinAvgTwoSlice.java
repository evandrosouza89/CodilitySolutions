...
public class MinAvgTwoSlice {
    ...
    public int solution(int[] A) {
        double currentAvg = (A[0] + A[1]) / 2d;
        int index = 0;

        double lastTwoAvg = (A[A.length - 2] + A[A.length - 1]) / 2d;

        for (int i = 0; i < A.length - 2; i++) {

            double twoAvg = (A[i] + A[i + 1]) / 2d;
            double threeAvg = (A[i] + A[i + 1] + A[i + 2]) / 3d;

            if (twoAvg < currentAvg) {
                currentAvg = twoAvg;
                index = i;
            }
            if (threeAvg < currentAvg) {
                currentAvg = threeAvg;
                index = i;
            }
        }

        if (lastTwoAvg < currentAvg) {
            index = A.length - 2;
        }
